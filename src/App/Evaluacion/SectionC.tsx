...
const tasks = ["Cosecha", "Empaque (campo)", "Empaque (bodega)", "Ensamblador de cajas", "Plantación", "Esquivada", "Conductor de maquinaria", "Fletero", "Deshierbada", "Piedra", "Piochada", "Otro"];
const bodegaTasks = new Set(["Empaque (bodega)", "Ensamblador de cajas", "Esquivada", "Otro"]);
const fieldClass = "mt-1 block w-full rounded-lg border-2 border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-900 outline-none transition focus:border-secondary focus:ring-2 focus:ring-primary/30";
...
export type SectionCData = {
  evaluationDate: string; fieldNumber: string; crop: string;
  weatherConditions: string; terrainConditions: string;
  harvestNumber: "1" | "2" | "3" | ""; task: string; otherTask: string;
  taskSpecification: string; quantity: string; unit: string; observations: string;
  finalRating: Rating | "";
};

// eslint-disable-next-line react-refresh/only-export-components
export const emptySectionCData: SectionCData = {
  evaluationDate: getLocalDate(), fieldNumber: "", crop: "", weatherConditions: "",
  terrainConditions: "", harvestNumber: "", task: "", otherTask: "",
...
export default function SectionC({ data, workType, onChange, onBack, onSubmit }: Props) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); onSubmit(); }} className="space-y-6">
    ...
    <fieldset className="space-y-4">
      ...
      <ChoiceGrid title="Tipo de cultivo" name="crop" options={availableCrops} value={data.crop} onChange={(value) => update("crop", value)} />
      {workType === "campo" && <div className="grid gap-4 sm:grid-cols-2">
          <TextField label="Condiciones meteorológicas" name="weatherConditions" value={data.weatherConditions} onChange={(value) => update("weatherConditions", value)} placeholder="Ej.: soleado, lluvia, nublado…" />
          <TextField label="Condiciones del terreno" name="terrainConditions" value={data.terrainConditions} onChange={(value) => update("terrainConditions", value)} placeholder="Ej.: subida, lodo, plano…" />
        </div>}
      <ChoiceGrid title="Número de cosecha" name="harvestNumber" options={["1", "2", "3"]} labels={{ "1": "1ra", "2": "2da", "3": "3era" }} value={data.harvestNumber} onChange={(value) => update("harvestNumber", value as SectionCData["harvestNumber"])} />
      <ChoiceGrid title="Tarea" name="task" options={availableTasks} value={data.task} onChange={(value) => update("task", value)} />
    ...
    <fieldset className="rounded-lg border border-gray-200 p-4">
      ...
      <div className="grid gap-4 sm:grid-cols-[1fr_1fr_1.25fr]">
        <div><p className="text-sm font-semibold text-gray-800">Duración</p><div className="mt-1 flex min-h-11 items-center rounded-lg border-2 border-gray-200 bg-gray-50 px-3 text-sm font-bold text-secondary">15 minutos</div></div>
        <TextField label="Cantidad" name="quantity" type="number" value={data.quantity} onChange={(value) => update("quantity", value)} placeholder="0" min="0" step="0.01" required />
...
type TextFieldProps = { label: string; name: string; value: string; onChange: (value: string) => void; type?: string; placeholder?: string; required?: boolean; min?: string; step?: string };
function TextField({ label, onChange, ...inputProps }: TextFieldProps) { return <label className="text-sm font-semibold text-gray-800">{label}<input {...inputProps} onChange={(event) => onChange(event.target.value)} className={fieldClass} /></label>; }

type ChoiceGridProps = { title: string; name: string; options: string[]; labels?: Record<string, string>; value: string; onChange: (value: string) => void };
function ChoiceGrid({ title, name, options, labels, value, onChange }: ChoiceGridProps) { return <fieldset><legend className="mb-2 text-sm font-semibold text-gray-800">{title}</legend><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{options.map((option) => <label key={option} className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm transition ${value === option ? "border-secondary bg-primary/15 font-semibold text-deepgreen" : "border-gray-300 hover:bg-tertiary"}`}><input type="radio" name={name} value={option} checked={value === option} onChange={() => onChange(option)} required className="size-4 accent-secondary" />{labels?.[option] ?? option}</label>)}</div></fieldset>; }
